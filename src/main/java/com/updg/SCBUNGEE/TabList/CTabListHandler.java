package com.updg.SCBUNGEE.TabList;
 ...
 * Date: 15.12.13  21:30
 */
public class CTabListHandler extends TabListAdapter implements CustomTabList {
    ...
    public CTabListHandler(ProxiedPlayer player) {
        this.init(player);
    }
    ...
    public synchronized void clear() {
        for (ProxiedPlayer p : ProxyServer.getInstance().getPlayers())
            getPlayer().unsafe().sendPacket(new PlayerListItem(p.getDisplayName(), false, (short) 9999));
        for (int i = 0; i < ROWS; i++) {
            for (int j = 0; j < COLUMNS; j++) {
                if (sent[i][j] != null) {
